...
conflicts: ($) => [
  ...
  [$.call_expression],
	...
	rules: {
		source_file: ($) => seq(repeat($._statement), $._terminator),
...
assignment_expression: ($) =>
  prec.right(seq(choice($.variable_expression, $.index_expression), "=", $._expression)),
  ...
  choice(
    ...
    prec.left(
      ...
      seq($._expression, choice("==", "!=", "<", "<=", ">", ">="), $._expression),
		...
		number_literal: ($) => choice(hex_literal, oct_literal, decimal_literal, binary_literal),
...
variant_pattern: ($) =>
  seq(
    ...
    optional(seq("(", repeat(seq($._identifier, optional(seq(",", $._terminator)))), ")")),
		...
		interpolated_string: ($) =>
			seq('"', repeat(choice(/[^\\"$]+/, seq("\\", /./), seq("${", $._expression, "}"))), '"'),
